...
pickle_in = open("verified_tickers.data", "rb")
verified_tickers = pickle.load(pickle_in)

#Ticker symbol input list
Symbols = verified_tickers
...
for i in Symbols: 
	...
	#drop unused columns
	stock_final.drop(['High','Low','Adj Close', ],axis=1,inplace=True)
	...
	stock_final['Overnight Total Return'] = (stock_final['Overnight Return %']+1).cumprod()-1
	
	#calculate compounding intraday returns
	stock_final['Intraday Total Return'] = (stock_final['Intraday Return %']+1).cumprod()-1
	...
	if overnight_total > long and long > 0:
